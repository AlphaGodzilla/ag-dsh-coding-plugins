...
技能仅对模型开放（`modelInvocable: true, userInvocable: false`），且只在宿主已挂载
...
- `git-commit-zh` 技能仅在宿主挂载 `skills` 服务时注册（`ctx.inject`），只对模型开放
  （`modelInvocable: true, userInvocable: false`）；技能正文来自 `prompts/skill-commit.md`。
